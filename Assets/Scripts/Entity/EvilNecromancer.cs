...
public class EvilNecromancer : EnemyEntity
{
    ...
    void FixedUpdate()
    {
        ...
        {
            StartCoroutine(HomingFireball());
            nextFireballTime = Time.time + fireballCooldown;
        }
    }

    IEnumerator HomingFireball()
    {
        yield return new WaitForSeconds(1f); // Wait for 1 second before making the fireball home in
        ...
        // Set the fireball's initial direction
        ...
        // Continue homing until the fireball is destroyed or reaches the player
        while (isHoming && fireball != null)
        {
            // Adjust the fireball's direction towards the player in each frame
            direction = (targetPlayer.transform.position - fireball.transform.position).normalized;
