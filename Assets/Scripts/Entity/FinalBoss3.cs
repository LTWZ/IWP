...
public class FinalBoss3 : EnemyEntity
{
    ...
    private float phase6Timer = 3.0f;
    ...
    public bool hasenemyspawned = false;
    ...
    void UpdatePhase5()
    {
        ...
        {
            phase5Timer = 10.0f;
            currentState = EnemyState.Phase6;
    ...
    void UpdatePhase6()
    {
        phase6Timer -= Time.deltaTime;
        Debug.Log("Phase 6 Timer: " + phase6Timer);
    ...
    void FixedUpdate()
    {
        ...
        if (!isMoving && Vector2.Distance(rb.position, targetPlayer.transform.position) <= 20)
        {
            ...
            if (!hasenemyspawned)
            {
                if (currentState == EnemyState.Phase6)
                {
    ...
    void SummonRandomEnemies()
    {
        int enemiesToSummon = 3; // Change this number as needed
        ...
        for (int i = 0; i < enemiesToSummon; i++)
        {
            ...
            // Calculate a random position around the boss (adjust the radius as needed)
            Vector2 randomPosition = (Vector2)transform.position + Random.insideUnitCircle * summonRadius;

            // Instantiate the enemy at the calculated position
            GameObject summonedEnemy = Instantiate(enemyPrefab, randomPosition, Quaternion.identity);

            // Optionally, you can set up any additional properties or scripts for the summoned enemies
            // summonedEnemy.GetComponent<YourEnemyScript>().SetupMethod();
        }
    }
